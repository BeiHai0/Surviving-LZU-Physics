...
% ===== 数学宏包 =====
\usepackage{amsmath, amssymb, amsthm, mathtools, bm, physics, braket, xcolor, tikz, pgfplots} %"braket"--狄拉克符号宏包；"xcolor"--文字颜色宏包
\pgfplotsset{compat=1.18}
\usetikzlibrary{decorations.markings} % 用于路径上的箭头
...
\fancyhead[RE]{\rightmark} % 偶数页页眉显示节名
\fancyhead[LO]{\rightmark} % 奇数页页眉显示节名
\fancyfoot[C]{\thepage} % 页码在底部居中

% ===== 定理和例题等环境 =====
\theoremstyle{plain} % plain 风格：标题加粗，正文斜体
...
% ===== 证明和解等环境 =====
\theoremstyle{definition}  % definition 风格：标题加粗，正文直立
