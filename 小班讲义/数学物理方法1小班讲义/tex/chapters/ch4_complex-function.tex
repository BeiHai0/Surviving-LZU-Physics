...
下面利用极限法计算 $f(z) $ 在回路 $l $ 内孤立奇点处的留数。

$f(z) $ 在一阶极点 $z_1=\mathrm{i} $ 处的留数：

\begin{equation}
\begin{split}
\mathrm{Res} f(z_1)
&=\frac{1}{0!}\lim_{z\to z_1}\frac{\mathrm{d}^0}{\mathrm{d}z^0} \left[(z-z_1)\cdot f(z) \right] \\
&=\lim_{z\to \mathrm{i}} (z-\mathrm{i})\cdot\frac{1}{(z+\mathrm{i})(z-\mathrm{i})(z-1)^2} \\
...
$f(z) $ 在二阶极点 $z_2=1 $ 处的留数：

\begin{equation}
\begin{split}
\mathrm{Res}f(z_2)
&=\frac{1}{1!}\lim_{z\to z_2} \frac{\mathrm{d}^1}{\mathrm{d}z^1}\left[(z-z_2)^2\cdot f(z)\right] \\
&=\lim_{z\to 1} \frac{\mathrm{d}}{\mathrm{d}z}\left[(z-1)^2\cdot \frac{1}{(z+\mathrm{i})(z-\mathrm{i})(z-1)^2} \right] \\
...
\subsubsection{计算一类被积函数为有理三角函数式的实积分}
...
\begin{equation}
z
=\cos\theta+\mathrm{i}\sin\theta,\quad
z^{-1}
...
令 $\displaystyle{f(z)=\frac{1}{\varepsilon z^2+2z+\varepsilon} }$ ，利用求根公式，可得被积函数的两个一阶极点：

\begin{equation}
z_1
=\frac{-1+\sqrt{1-\varepsilon^2}}{\varepsilon},\quad
z_2
...
\begin{equation}
\mathrm{d}\theta = \frac{\mathrm{d}z }{\mathrm{i}\mathrm{e}^{\mathrm{i}\theta} } = \frac{\mathrm{d}z }{\mathrm{i}z },\quad
\cos\theta = \frac{1 }{2 }\left(z+z^{-1} \right),\quad
\sin\theta = \frac{1 }{2\mathrm{i} } \left(z-z^{-1} \right).
\end{equation}

设 $C$ 是复平面上的单位圆，则待求积分 $I $ 可表达为：
...
计算 $\displaystyle{I=\int_0^{+\infty} \frac{\ln x }{\left(1+x^2 \right)^2 } \mathrm{d}x }$. 

\end{example}

\begin{solution}
    
若把 $\ln z $ 看成复平面到复平面的映射，则这个映射是多值的。为了消除多值性，我们把 $\ln z $ 看成黎曼面到复平面的映射。

考虑如下的 keyhole 围道：

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale = 0.8, >={Kite[inset=0pt,length=0.32cm,bend]},
    decoration={markings,
...
实轴上方的路径记为 $C_1 $，下方的路径记为 $C_2 $。

构造如下的函数：

\begin{equation}
f(z)
=\frac{\ln^2 z }{\left(1+z^2 \right)^2 }
=\frac{\ln^2 z }{(z+\mathrm{i})^2\left(z-\mathrm{i} \right)^2 } ,\quad \arg z \in (0,2\pi),\text{而非}\quad
f(z)
=\frac{\ln z }{\left(1+z^2 \right)^2 }.
\end{equation}

当 $\delta\to 0^+,R\to +\infty $ 时，$f(z) $ 在围道内的奇点为 $z_1=\mathrm{i},z_2=-\mathrm{i} $，其中 $z_1,z_2 $ 都是二阶极点。

计算 $f(z) $ 在 $z_1=\mathrm{i} $ 处的留数：

\begin{equation}
\begin{split}
\mathrm{Res}f(z_1)
&=\frac{1 }{1! } \lim_{z\to z_1} \frac{\mathrm{d}^1 }{\mathrm{d}z^1 } \left[(z-z_1)^2 f(z) \right] \\
&=\frac{-4\pi + \pi^2 \mathrm{i} }{16 },
\end{split}
\end{equation}

计算 $f(z) $ 在 $z_2=-\mathrm{i} $ 处的留数：

\begin{equation}
\begin{split}
\mathrm{Res}f(z_2)
&=\frac{1 }{1! } \lim_{z\to z_2} \frac{\mathrm{d}^1 }{\mathrm{d}z^1 } \left[(z-z_2)^2 f(z) \right] \\
&=\frac{12\pi - 9 \pi^2 \mathrm{i} }{16 },
\end{split}
\end{equation}
...
\begin{equation}
\lim_{\delta\to 0^+,R\to +\infty} \left(\int\limits_{C_R} + \int\limits_{C_\delta} + \int\limits_{C_1} + \int\limits_{C_2} \right) f(z)\mathrm{d}z
=2\pi \mathrm{i} \left(\mathrm{Res}f(z_1) + \mathrm{Res}f(z_2) \right),
\end{equation}

可以证明，当 $\delta\to 0^+,R\to +\infty $ 时，大圆弧和小圆弧上的积分为零。

并且，沿 $C_1 $ 的积分与沿 $C_2 $ 的积分有关系，具体如下：

\begin{equation}
\begin{split}
\lim_{\delta\to 0^+,R\to +\infty} \int\limits_{z'\in C_2} f(z') \mathrm{d}z'
&=\lim_{\delta\to 0^+,R\to +\infty} \int\limits_{z\in C_1} f\left(z \mathrm{e}^{\mathrm{i} 2\pi} \right) (-\mathrm{d}z) \\
&=\lim_{\delta\to 0^+,R\to +\infty} -\int\limits_{z\in C_1} \frac{\ln^2\left(z \mathrm{e}^{\mathrm{i}2\pi} \right) }{\left(1+z^2 \right)^2 } \mathrm{d}z \\
&=\lim_{\delta\to 0^+,R\to +\infty} -\int\limits_{z\in C_1} \frac{\left(\ln z + \mathrm{i}2\pi \right)^2 }{\left(1+z^2 \right)^2 } \mathrm{d}z \\
&=\lim_{\delta\to 0^+,R\to +\infty} \int\limits_{z\in C_1} \frac{-\ln^2 z -4\pi \mathrm{i} \ln z + 4\pi^2 }{\left(1+z^2 \right)^2 } \mathrm{d}z,
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\lim_{\delta\to 0^+,R\to +\infty} \left(\int\limits_{C_1} + \int\limits_{C_2} \right) f(z) \mathrm{d}z
&=\lim_{\delta\to 0^+,R\to +\infty} -4\pi \mathrm{i} \int\limits_{C_1} \frac{\ln z }{\left(1+z^2 \right)^2 }\mathrm{d}z + 4\pi^2 \int\limits_{C_1} \frac{1 }{\left(1+z^2 \right)^2 } \mathrm{d}z \\
&=-4\pi \mathrm{i} I + 4\pi^2 A. 
\end{split}
\end{equation}
