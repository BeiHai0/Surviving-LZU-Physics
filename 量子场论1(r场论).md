...
假设 $\hat{H}_S $ 不含时，在海森堡绘景中，态矢定义为：

$$
\Psi_H
\equiv \mathrm{e}^{\mathrm{i } \hat{H}_S t} \Psi_S
$$

可以验证：

$$
\frac{\mathrm{d} \Psi_H }{\mathrm{d}t } 
=0
$$
...
$$
\begin{aligned}
\frac{\mathrm{d} \hat{F}_H }{\mathrm{d}t } 
=\mathrm{i} \left[\hat{H} , \hat{F}_H \right]
\end{aligned}
$$
...
$$
\hat{\phi}^{(\pm)}(x)
=\left(\frac{1 }{2\pi }  \right)^{3/2} \int \mathrm{e}^{\mp\mathrm{i}k x}\frac{1 }{\sqrt{2\varepsilon_{\vec{k}}} } \hat{\phi}^{(\pm)}\left(\vec{k} \right) \mathrm{d}^3\vec{k}
$$
