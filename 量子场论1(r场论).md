...
\begin{aligned}
\hat{H}
=\frac{1 }{2 } \int \varepsilon_k \left\{\hat{a}^{(+)}\left(\vec{k} \right) , \hat{a}^{(-)}\left(\vec{k} \right) \right\} \mathrm{d}^3\vec{k}
\end{aligned}
$$
...
$$
\hat{H}
=\frac{1 }{2 } \sum_{\vec{k}} \varepsilon_k \left\{\hat{a}^{(+)}_{\vec{k}} , \hat{a}^{(-)}_{\vec{k}} \right\}
$$
