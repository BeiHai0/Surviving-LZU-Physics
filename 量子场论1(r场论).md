...
&=\frac{-1 }{\left(2\pi \right)^n } \int \mathrm{d}^4 k \frac{\mathrm{e}^{\mathrm{i}k(x-x')} }{a_0 + \mathrm{i}a_\mu k_\mu + \mathrm{i}^2 a_{\mu\nu} k_\mu k_\nu + \cdots } 
\end{aligned}
$$
...
$$
\Delta^G(x-x')
=\left(\frac{1 }{2\pi }  \right)^4 \int \Delta^G(k) \mathrm{e}^{\mathrm{i}k(x-x')} \mathrm{d}^4 k
$$
...
用 $\hat{U}_\alpha^{(\pm)}(x) $ 表示广义场函数

用 $\hat{\bar{U}}_\alpha^{(\pm)}(x) $ 表示广义场函数的共轭

用 $Q_{\alpha\beta}^{(\pm)}(x) $ 表示
