...
对于自由旋量粒子，其具有确定四维动量 $p_\mu $，其波函数 $\psi $ 应是四维动量算符本征波函数，即 $\psi $ 满足 $\hat{p}_\mu $ 的本征方程：
...
自由旋量粒子波函数 $\psi $ 是一种特殊的旋量波函数，其应当满足 Dirac 方程。将 $\psi $ 的平面波解代入自由旋量场 Dirac 方程 $\left(\gamma_\mu\partial_\mu + m \right)\psi = 0$，可得：
