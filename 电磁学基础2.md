...
将 $f(\vec{x}-\vec{x}') $ 看作 $\vec{x}' $ 的函数（或者说看作 $x_1',x_2',x_3' $ 的函数），在 $\vec{x}'=\vec{0} $ 处展开：
...
&=f(\vec{x})+\sum_{i} x_i'\frac{\partial f(\vec{x}-\vec{x}')}{\partial (x_i-x_i')}\cdot\frac{\mathrm{d}(x_i-x_i')}{\mathrm{d} x_i'}\bigg|_{\vec{x}'=\vec{0}}
\end{aligned}
$$
...
$$
\varphi(\vec{x})
=\frac{1}{4\pi\varepsilon_0}\int\limits_{V}\rho(\vec{x}')\bigg[ \frac{1}{R}-\vec{x}'\cdot\frac{1}{R}+\frac{1}{2!}\sum_{i,j}x_i'x_j'\frac{\partial^2}{\partial x_i\partial x_j}\frac{1}{R}+\cdots  \bigg] \mathrm{d}V'
...
$$
\vec{A}(\vec{x})
=\frac{\mu}{4\pi}\int\limits_{V} \frac{\vec{J}(\vec{x}')}{r}\mathrm{d}V'
$$
