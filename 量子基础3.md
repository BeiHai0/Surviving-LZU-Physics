...
设在 $\Ket{b_r b_s \cdots b_v } $ 中，$b_1 $ 出现 $n_1 $ 次，$b_2 $ 出现 $n_2 $ 次，$b_l $ 出现 $n_l $ 次，定义：
...
引入 Zeeman 项（反应local moments 与 磁场 $B_0=\mu_0 H_0 $ 对哈密顿量的贡献）的海森堡模型哈密顿量可写为：
