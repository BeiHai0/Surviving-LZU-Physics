...
考虑 $n $ 个数 $1,2,\cdots,n $ 的一个排列 $\left(1,2,\cdots,n \right) $，设 $\left(a_1,a_2,\cdots,a_n \right) $ 是这 $n $ 个数的一种重新排列，则这种排列对应的置换记为
...
用 $p(\sigma) $ 表示置换 $\sigma $ 的置换次数（从 $\left(1,2,\cdots, \right) $ 到 $\left(a_1,a_2,\cdots,a_n \right) $ 所需两两交换的次数）。
...
=\ket{b_1}_{a_1}\otimes \ket{b_2}_{a_2}\otimes \cdots \otimes \ket{b_n}_{a_n}
$$
...
$$
\ket{b_1}_1\otimes\ket{b_2}_2\otimes\ket{b_3}
$$
...
&=\ket{b_1}_2\otimes\ket{b_2}_1\otimes\ket{b_3}_3 \\
&=\ket{b_2}_1 \otimes \ket{b_1}_2 \otimes \ket{b_3}_3
\end{aligned}
$$
...
$$
P_\sigma
=\sum_{b_1'\in\mathrm{Spec}\left(B_1 \right)}\cdots \sum_{b_n'\in\mathrm{Spec}\left(B_n \right)} \left(\ket{b_1'}_{a_1} \otimes \cdots \otimes \ket{b_n'}_{a_n} \right)\left(\bra{b_1'} \otimes \cdots \otimes \bra{b_n'} \right)
$$
...
&=\left[\sum_{b_1'\in\mathrm{Spec}\left(B_1 \right)}\cdots \sum_{b_n'\in\mathrm{Spec}\left(B_n \right)} \left(\ket{b_1'}_{a_1} \otimes \cdots \otimes \ket{b_n'}_{a_n} \right)\left(\bra{b_1'} \otimes \cdots \otimes \bra{b_n'} \right) \right]\left(\ket{b_1}_1 \otimes \cdots \otimes \ket{b_n}_n \right) \\
&=\sum_{b_1'\in\mathrm{Spec}\left(B_1 \right)}\cdots \sum_{b_n'\in\mathrm{Spec}\left(B_n \right)} \left(\ket{b_1'}_{a_1} \otimes \cdots \otimes \ket{b_n'}_{a_n} \right) \delta_{b_1',b_1}\cdots \delta_{b_n',b_n} \\
&=\ket{b_1}_{a_1} \otimes \cdots \otimes \ket{b_n}_{a_n}
\end{aligned}
$$
...
首先在 $m $ 个全同粒子的 Hilbert 空间中定义真空态（一个什么粒子都没有的状态）$\ket{0} $：
