...
考虑 $n $ 个数 $1,2,\cdots,n $ 的顺序排列 $\left(1,2,\cdots,n \right) $，这 $n $ 个数的一个置换，记为 $\sigma $，定义为这 $n $ 个数的一个重新排列。特别地，把置换 $\sigma $ 第 $i $ 位置上的数记为 $\sigma(i) $，则置换 $\sigma $ 形式上可写为：

$$
\sigma
\equiv \left(\sigma(1),\sigma(2),\cdots,\sigma(n) \right)
$$


$\left(1,2,\cdots,n \right) $ 共有 $n! $ 种排列方式，因此有 $n! $ 种置换。把 $n $ 个不同元素的所有置换构成的集合记为 $S_n $。

用 $p(\sigma) $ 表示置换 $\sigma $ 的置换次数（从 $\left(1,2,\cdots, \right) $ 到 $\left(\sigma(1),\sigma(2),\cdots,\sigma(n) \right) $ 所需两两交换的次数）。
...
$$
\sigma
=\left(\sigma(1),\sigma(2),\sigma(3) \right)
=(3,1,2)
$$
...
\begin{aligned}
\braket{n;b'_1 \cdots b'_n | n;b_1 \cdots b_n}
&=\left(\frac{1 }{n! }  \right)^2 \sum_{\sigma'} \sum_{\sigma} \varepsilon^{p(\sigma')} \varepsilon^{\sigma} P_{\sigma'} P_{\sigma} {\ } \smash{\raisebox{-0.6ex}{$\scriptstyle 1$}}\hspace{-0.1em} \braket{b'_1|b_1}_1 \cdots \smash{\raisebox{-0.6ex}{$\scriptstyle n$}}\hspace{-0.1em} \braket{b'_1|b_1}_n \\
&=\left(\frac{1 }{n! }  \right)^2 \sum_{\sigma} P_{\sigma} \varepsilon^{\sigma} \sum_{\sigma'} \varepsilon^{p(\sigma')} P_{\sigma'} {\ } \smash{\raisebox{-0.6ex}{$\scriptstyle 1$}}\hspace{-0.1em} \braket{b'_1|b_1}_1 \cdots \smash{\raisebox{-0.6ex}{$\scriptstyle n$}}\hspace{-0.1em} \braket{b'_1|b_1}_n \\
&=
\end{aligned}
$$
