...
体积 $V$ 内，$\varepsilon\sim \varepsilon+\mathrm{d}\varepsilon$ 的能量范围内，玻色子的量子态数为：
...
$$
n_0(T)+\frac{2\pi}{h^3}(2m)^{\frac{3}{2}}\int_{0}^{+\infty} \frac{\varepsilon^\frac{1}{2}\mathrm{d}\varepsilon}{e^{\frac{\varepsilon}{kT_c}}-1}
=n
$$

其中，$n_0(T)$ 是温度 $T$ 时处在能级 $\varepsilon=0$ 的粒子数密度
...
在温度为绝对零度时，玻色粒子将全部处在 $\varepsilon=0$ 的最低能级，在 $T<T_c$ 时，有宏观量级的粒子在能级 $\varepsilon=0$ 凝聚，这一现象称为玻色-爱因斯坦凝聚
...
体积为 $V$ 的空窖内，$p\sim p+\mathrm{d}p$ 的动量范围内，光子的量子态数为：

$$
\frac{8\pi V}{h^3} p^2\mathrm{d}p
$$

体积为 $V$ 的空窖内，$\omega\sim \omega+\mathrm{d}\omega$ 的圆频率范围内，光子的量子态数为：
...
=\frac{V}{\pi^2 c^3} \frac{\hbar\omega^3}{e^{\frac{\hbar\omega}{kT}}-1}\mathrm{d}\omega
$$
...
$$
E
=\frac{i=1}{3N}\frac{p_{\xi_i^2}}{2m}+\frac{1}{2}\sum_{i,j} a_{ij} \xi_i\xi_j +\phi_0
$$
...
$$
E
=\frac{1}{2}\sum_{i=1}^{3N}(p_i^2+\omega_i^2+q_i^2)+\phi_0
$$
...
&=e^{-\beta \phi_0} \sum_{\{ n_i \}} e^{-\beta \sum\limits_i \hbar\omega_i(n_i+\frac{1}{2})} \\
&=e^{-\beta\phi_0} \sum_{\{n_i\}} \prod_i e^{-\beta\hbar\omega_i(n_i+\frac{1}{2})} \\
&=e^{-\beta \phi_0} \prod_i \sum_{n_i=0}^{\infty} e^{-\beta \hbar\omega_i(n_i+\frac{1}{2})} \\
&=e^{-\beta\phi_0}\prod_i \frac{e^{-\frac{\beta\hbar\omega_i}{2}}}{1-e^{-\beta\hbar\omega_i}}
\end{aligned}
$$
...
固体上传播的弹性波有纵波和横波两种，以 $c_l,c_t$ 分别表示纵波和横波的传播速度
...
在 $\omega \sim \omega+\mathrm{d}\omega$ 的范围内，简正振动数为：

$$
D(\omega)\mathrm{d}\omega
=\frac{V}{2\pi^2}(\frac{1}{c_l^3+c_t^3})\omega^2\mathrm{d}\omega
...
$$
U
=U_0+\int_{0}^{\omega_D} \hbar\omega\cdot f\cdot D(\omega)\mathrm{d}\omega
