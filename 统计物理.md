...
体积 $V$ 内，$p_x$ 到 $p_x+\mathrm{d}p_x$，$p_y$ 到 $p_y+\mathrm{d}p_y$，$p_z$ 到 $p_z+\mathrm{d}p_z$ 的动量范围内，分子数为：
...
$$
\frac{V}{h^3}\iiint e^{-\alpha-\frac{1}{2mkT}(p_x^2+p_y^2+p_z^2)}\mathrm{d}p_x\mathrm{d}p_y\mathrm{d}p_z
=N
$$
...
体积 $V$ 内，$v_x$ 到 $v_x+\mathrm{d}v_x$，$v_y$ 到 $v_y+\mathrm{d}v_y$，$v_z$ 到 $v_z+\mathrm{d}v_z$ 的速度范围内，分子数为：

$$
N(\frac{m}{2\pi kT})^{\frac{3}{2}}e^{-\frac{m}{2kT}(v_x^2+v_y^2+v_z^2)}\mathrm{d}v_x\mathrm{d}v_y\mathrm{d}v_z
$$

单位体积内，$v_x$ 到 $v_x+\mathrm{d}v_x$，$v_y$ 到 $v_y+\mathrm{d}v_y$，$v_z$ 到 $v_z+\mathrm{d}v_z$ 的速度范围内，分子数为：
