...
\varepsilon_{n_x}
=\frac{p_x^2}{2m}
=\frac{2\pi^2\hbar^2}{m}\cdot\frac{n_x^2}{L^2}
$$
...
猜想：运用拉格朗日乘数法求函数 $f(x,y)$ 的极值点的前提是：$f(x,y)$ 的表达式中不含有常数（含有常数会有很多诡异的现象）好像不对
...
\begin{aligned}
\ln \Omega
&\approx N(\ln N-1)-\sum_{l} a_l(\ln a_l-1)+\sum_{l} a_l\ln \omega_l \\
