...
到原点 $O(0,0) $ 的线长（间隔）为常数 $K $ 的所有点 $(t,x) $ 的集合称为等线长曲线，满足方程：
...
当 $K<0 $ 时，$\displaystyle{\frac{t^2 }{\sqrt{-K}^2 }  }-\frac{x^2 }{\sqrt{-K}^2 }=1 , t=\pm\sqrt{x^2-K} $，曲线上任一点到原点 $O $ 的线长相等。该曲线又称为校准曲线。
...
两同时同地事件在任何惯性系中仍为同时同地事件。

两同时不同地时间 $S^2=\Delta x^2>0 $，类空，同时是相对的。两类空事件的同时和先后都是相对的。
