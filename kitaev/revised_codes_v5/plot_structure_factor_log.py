...
for Kx, Ky, Kz in K_list:
    for kappa in kappa_list:
        ...
        for h in h_list:
            ...
            plt.colorbar(label=r'$S(\omega,\boldsymbol{q})$')
            plt.xticks(q_dist[node_idx], labels)
            ax = plt.gca()
            ...
            info = mpatches.Patch(
                color='none',
                label=rf'$(K,\kappa,h,\eta)=({Kx},{fmt_kappa(kappa)},{fmt_h(h,h_c)},{eta:.2f})$'
            )
            
            plt.legend(handles=[info], loc='upper right', frameon=False, labelcolor='white')
            plt.tight_layout()
            save_dir = (
                Path("kitaev_data")
