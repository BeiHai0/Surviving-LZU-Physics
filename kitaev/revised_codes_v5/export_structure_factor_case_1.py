...
def _worker_q(q_idx):
    ...
    weights = np.zeros(3 * N, dtype=np.float64)
    for mu in [1, 2, 3]:
...
def main():
    ...
    # parallel config
    # =========================
    nproc = int(os.environ.get("SLURM_CPUS_PER_TASK", 1)) // 2
    ...
    eigvals_all = np.zeros((len(q_list), 3 * N), dtype=np.float64)
    weights_all = np.zeros((len(q_list), 3 * N), dtype=np.float64)
    ...
    # =========================
        ...
        with ctx.Pool( processes=nproc, initializer=_init_worker, initargs=(global_dict,)) as pool:
        
            for q_idx, eigvals, weights in pool.imap_unordered(_worker_q, range(len(q_list)), chunksize=1):
    ...
    manager.save_data(f'q_dist_q_interval_number_{q_interval_number}',
                      q_dist, N1, N2, bc1, bc2, **params2)
