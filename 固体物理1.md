...
晶格位移矢量 $u_{lj\sigma} $ 表示第 $l $ 个元胞中第 $j $ 个原子极化方向为 $\sigma $ 的位移，令该原子的质量的为 $m_j $
...
其中，$\rho(\omega) $ 称为声子态密度，它标势单位频率间隔内的模式数，满足总模式数等于总自由度数：
...
在 $\vec{q} $ 空间，色散关系 $\omega_s=\omega_s(\vec{q}) $  处于一系列连续的曲面 $S_\omega $，称为等频率面
...
=\frac{V}{(2\pi)^3}\sum_s \iint\frac{\mathrm{d} S\omega}{|\nabla_{\vec{q}}\omega_s(\vec{q}) |}
$$
...
=\frac{S}{(2\pi)^2}\sum_s \int \frac{\mathrm{d}l_\omega}{|\nabla_{\vec{q}}\omega_s(\vec{q}) |}
$$
...
=\frac{L}{2\pi}\sum_s \frac{2}{|\mathrm{d}\omega_s(q)/\mathrm{d}q |}
$$

### 爱因斯坦模型和爱因斯坦比热容

假定晶体中 $nN $ 个原子以同一确定频率振动（实际上是把晶体中的各种振动简化为**单一的长光学波模**），长光学波模的振动频率几乎不依赖于波矢，
...
对正点阵的近似，用一个半径为 $r_s $ 的 W-S 球来代替实际的 W-S 胞，且规定它们有相同体积：
...
$$
q_D
=\bigg( \frac{9\pi}{2} \bigg)^{1/3}\frac{1}{r_s} 
$$
...
$$
\rho_D(\omega)
=\frac{V}{(2\pi)^3}\sum_s \int\frac{\mathrm{d}S_\omega}{|\nabla_{\vec{q}}\omega_s(\vec{q}) |} 
=\left\{
\begin{aligned}
&\frac{9N\omega^2}{\omega_D^3}&&,~~0\leqslant \omega\leqslant \omega_D \\
&0&&,~~\omega>\omega_D
\end{aligned}
\right.
$$
...
&=\int_{0}^{\omega_D}\frac{9N\omega^2}{\omega_D^3}\cdot k_B\frac{\bigg( \frac{\hbar\omega}{k_B T} \bigg) ^2 e^{\hbar\omega/(k_B T)}}{[e^{\hbar\omega/(k_B T)}-1]^2}\mathrm{d}\omega
\end{aligned}
$$
...
$$
C_V^D
=9Nk_B \bigg( \frac{T}{\theta_D} \bigg)^{3}\int_{0}^{\theta/T} \frac{\xi^4 e^{\xi}}{(e^\xi-1)^2}\mathrm{d}\xi
=3Nk_B f(\frac{\theta_D}{T}) 
$$

$$
f(\frac{\theta_D}{T})
\equiv (\frac{T}{\theta_D})^3 \int_{0}^{\frac{\theta_D}{T}} \frac{\xi^4 e^{\xi}}{(e^\xi-1)^2}\mathrm{d}\xi
$$
...
状态密度：$\displaystyle{2V/(2\pi)^3 }$ 

能带电子态密度：

$$
N_n(E)
=\frac{2V}{(2\pi)^3}\int\limits_{\Omega^*} \mathrm{d}^3\vec{k}[E-E_n(\vec{k})]
$$
