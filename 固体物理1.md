...
在长波极限或连续介质的情况下，取 $q\to 0$，色散关系退化为：
...
此时，波的群速等于相速，表现为长波长弹性波。纵弹性波与声波是等同的。长声学波的轻重原子的振幅和复相位相同，它表示元胞质心的运动。
...
波矢密度=$\displaystyle{\frac{V}{(2\pi)^3} }$ 

一个独立的 $\vec{q} $ 和 $\omega_s(\vec{q}) $ 等价于 简正坐标 $Q_{\vec{q}s} $ 描述的谐振子，其能量本征值为：
...
$$
m_j \omega^2 A_{j\sigma\vec{q}} 
=\sum_{j'\sigma'} \lambda_{j\sigma j'\sigma'}(\vec{q}) A_{j'\sigma'\vec{q}}
$$

方程有解的条件是系数行列式为零：

$$
|\lambda_{j\sigma j'\sigma'}(\vec{q})-\omega^2m_j\delta_{jj'}\delta_{\sigma\sigma'} |
=0
$$

可解出 $3n $ 个色散关系 $\omega_s(\vec{q}),~~s=1,2,\cdots,3n $，其中，

有 $3 $ 支声学波（元胞质心自由度）

在布里渊区高对称点或连线上可以分为两支横波，一支纵波。

$(3n-3) $ 支光学波（元胞内原子相对运动自由度），在布里渊区高对称点或连线上可分为 $2(n-1) $ 支横波，$(n-1) $ 支纵波。
