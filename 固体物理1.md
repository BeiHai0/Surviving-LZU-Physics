...
&=\Re\{\sum_{j} C a_{ij}\mathrm{e}^{-\mathrm{i}\omega_j t} \}
\end{aligned}
$$

可见 $3N$ 个 $q_i$ 都以同样的频率振动（？）
...
$$
A_{j\sigma}\mathrm{e}^{-\mathrm{i}\omega_j t},~~~\sigma=1,2,3,~~u(\vec{0})=A_{j\sigma}
$$

其中，$A_{j\sigma} $ 为该原子在偏振方向 $\sigma(1,2,3)$ 的振幅。

由晶格的平移对称性，每个原子在相同偏振方向的振幅 $u(\vec{R}_l)$ 必须相同，但可相差一个相位因子 $\lambda(\vec{r}) $
