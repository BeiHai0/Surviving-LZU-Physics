...
对于一个确定的 $\vec{k} $，有无穷多个分立的 $E_n(\vec{k}) $ 和 $\psi_{\vec{k}}^n(\vec{r}),~~n=1,2,\cdots $

对于一个确定的 $n $，$E_n(\vec{k}) $ 是倒空间的周期函数，$E_n(\vec{k}+\vec{K}_h)=E_n(\vec{k}) $
...
因此，独立的 $\vec{k} $ 可限制在 $\mathrm{1^{st}BZ} $ 内
...
$$
\ket{\vec{k}+\vec{K}_h}
=\frac{1}{\sqrt{N\Omega}}\mathrm{e}^{\mathrm{i}(\vec{k}+\vec{K}_h)\cdot\vec{r}}
$$
...
$$
\bigg[ -\frac{\hbar^2}{2m}\nabla^2+V(\vec{r})-E(\vec{k}) \bigg]\ket{\psi_{\vec{k}}}
=0 
$$
