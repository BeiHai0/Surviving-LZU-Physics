...
$$
S[f]
=\int_{x_1}^{x_2} \mathrm{d}x~L(x,f(x),f'(x),f''(x),\cdots,f^{(n)}(x),\cdots)
$$

这里被积函数 $L(x,f(x),f'(x),f''(x),\cdots,f^{(n)}(x),\cdots)$ 称为拉格朗日量，$S$ 称为作用量

#### 变分

函数本身的无穷小变化，以及由之引起的泛函的变化即变分

$$
\delta(f_1+f_2)
=\delta f_1+\delta f_2 \\
\delta(f_1 f_2)
=f_2\delta f_1+f_1\delta f_2 \\
\delta(\frac{f_1}{f_2})
=\frac{f_2\delta f_1-f_1\delta f_2}{f_2^2}
$$
