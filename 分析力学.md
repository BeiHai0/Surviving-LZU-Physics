...
考虑由质量为 $m_1$ 和 $m_2$ 的两个质点组成的系统，它们的相互作业为 $U$
...
拉格朗日量中没有出现 $\dot{\vec{R}}$，于是 $\vec{R}$ 是循环坐标，其对应的广义动量 $\vec{p}=\frac{\partial L}{\partial \dot{\vec{R}}}=(m_1+m_2)\dot{\vec{R}}$ 是守恒量，即体系静止或做匀速直线运动
...
\Longrightarrow
t
=\int_{r_0}^{r} \frac{\mathrm{d}r}{\sqrt{\frac{2}{m}(E-U(r)-\frac{l^2}{2mr^2})}}
$$
...
若力学量 $f$ 和 $g$ 是运动积分，则它们的泊松括号 $\{f,g \} $ 也是运动积分
