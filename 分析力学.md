...
循环坐标：若拉格朗日量中不含某个广义坐标，则这个广义坐标称为循环变量（存在守恒量）
...
l
&=m|\vec{r}\times\dot{\vec{r}} | \\
&=m\bigg|[r\cos\theta\vec{e}_x+r\sin\theta \vec{e}_y]\times[(\dot{r}\cos\theta-r\dot{\theta}\sin\theta)\vec{e}_x+(\dot{r}\sin\theta+r\dot{\theta}\cos\theta)\vec{e}_y] \bigg| \\
&=mr^2\dot{\theta}
\end{aligned}
$$
...
（1）$\theta$ 为循环坐标，其相应的广义坐标是个守恒量，而 $\theta$ 对应的广义坐标恰为角动量：
...
质点从 $r_{\max}$ 运动至 $r_{\min}$ 在运动回 $r_{\max}$ 的时间间隔内，矢量转过的角度为：
...
- 将 $xyz$ 系绕 $z$ 轴旋转 $\phi(0\leqslant \phi\leqslant 2\pi)$ 得 $x'y'z$ 系；$\phi$ 称为旋进角
...
存在转动瞬心得必要条件为 $\vec{v}_A\cdot\vec{\omega}=0$
...
&=\omega_x\sum_{i}(y_i^2+z_i^2)-\omega_y\sum_{i} m_i x_i y_i-\omega_z\sum_{i} m_i x_iz_i
\end{aligned}
$$
...
-\sum\limits_{i} m_i y_i x_i &\sum\limits_{i}m_i(x_i^2+y_i^2) &-\sum\limits_{i}m_i y_i z_i \\
...
&\equiv \frac{1}{2}\sum_{i} m_i\vec{v}_i\cdot(\vec{\omega}\times\vec{v}_i) \\
...
适当选择坐标原点可以使得惯量张量为对角的，此时的转动惯量称为主转动惯量，$I_1=I_{11},I_2=I_{22},I_3=I_{33} $，相应的坐标轴称为惯量主轴，此时：
...
$$
I_1=I_2=
\frac{1}{M}\sum\limits_{i\leqslant j} m_i m_j l_{ij}^2 
$$ 
...
$$
\begin{aligned}
\sum\limits_{i\leqslant j} m_i m_j l_{ij}^2 
&=\sum_{i,j} m_i m_j [(z_i-z_C)-(z_j-z_C)]^2 \\
...
给定一个实单自变量函数 $f(t)$，假设它是一个凸函数，即其导函数 $f'(t)$ 严格点掉。将 $f'(t)$ 重新命名为一个新的实变量 $s$，若存在一个实函数 $g(s)$，使得 $g'(s)=t$，则称 $g(s)$ 是 $f(t)$ 的勒让德变换
