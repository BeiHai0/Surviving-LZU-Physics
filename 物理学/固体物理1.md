...
\begin{aligned}
\hat{H}
&=\frac{1}{2}\sum_{j}(\hat{P}_j^2+\omega_j^2\hat{Q}_j^2) \\
&=\frac{1}{2}\sum_j \bigg( -\hbar^2\frac{\partial^2}{\partial Q_j^2}+\omega_j^2Q_j^2 \bigg)
\end{aligned}
$$
...
$$
\varphi_{n_j}(Q_j)
=\frac{1}{\sqrt{\sqrt{\pi}2^{n_j} n_j!}}\bigg( \frac{\omega_j}{\hbar} \bigg)^{\frac{1}{4}}\exp(-\frac{\xi^2}{2})\mathrm{H}_{n_j}(\xi),~~\xi=\sqrt{\frac{\omega_j}{\hbar}} Q_j
$$
...
\boxed{
\omega_{\pm}^2(q)
=\beta\frac{m_1 m_2}{m_1+m_2}\bigg\{ 1\pm\sqrt{1-\frac{4m_1 m_2}{(m_1+m_2)^2}\sin^2(\frac{1}{2}aq)} \bigg\}
}
$$
...
晶格位移矢量 $u_{lj\sigma} $ 表示第 $l $ 个元胞中第 $j $ 个原子极化方向为 $\sigma $ 的位移，令该原子的质量的为 $m_j $（$l=1,2,\cdots,N,~~j=1,2,\cdots,n,~~\sigma=1,2,3 $）
...
$$
\lambda_{lj\sigma,l'j'\sigma'}
=\lambda_{jj'\sigma\sigma'}(\vec{R}_l-\vec{R}_{l'})
$$
...
$$
u_{lj\sigma}
=A_{j\sigma \vec{q}}\mathrm{e}^{\mathrm{i}(\vec{q}\cdot\vec{R}_l-\omega t)}
$$
...
$$
m_j\omega^2 A_{j\sigma\vec{q}}\mathrm{e}^{\mathrm{i}(\vec{q}\cdot\vec{R}_l-\omega t)}
=\sum_{l'j'\sigma'} A_{j'\sigma' \vec{q}}\lambda_{j\sigma j'\sigma'}(\vec{R}_l-\vec{R}_{l'})\mathrm{e}^{\mathrm{i}(\vec{q}\cdot\vec{R}_{l'}-\omega t)}
$$

定义：

$$
\lambda_{j\sigma j'\sigma'}(\vec{q})
\equiv \sum_{l} \lambda_{j\sigma j'\sigma'}(\vec{R}_l)\mathrm{e}^{-\mathrm{i}\vec{q}\cdot\vec{R}_l}
$$

方程有解的条件为：

$$
|\lambda_{j\sigma j'\sigma'}(\vec{q})-\omega^2 m_j\delta_{jj'}\delta_{\sigma\sigma'} |_{3n\times 3n}
=0
$$
...
$$
\varepsilon_{\vec{q}s}
=(n_{\vec{q}s}+\frac{1}{2})\hbar\omega_{s}(\vec{q})
$$
