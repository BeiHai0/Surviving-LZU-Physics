...
$$
V(\vec{r})
=\sum_{\vec{K}_h}V\left(\vec{K}_h\right)\mathrm{e}^{\mathrm{i}\vec{K}_h\cdot\vec{r}}
$$

$$
V\left(\vec{K}_h\right)
...
周期场中单电子哈密顿量本征态是一系列相差一个倒格矢的平面波的叠加
...
\begin{aligned}
V(\vec{r})
&=\sum_{\vec{K}_h} V(\vec{K}_h)\mathrm{e}^{\mathrm{i}\vec{K}_h\cdot\vec{r}} \\
&=\bar{V}+\sum_{\vec{K}_h\ne \vec{0}} \mathrm{e}^{\mathrm{i}\vec{K}_h\cdot\vec{r}} \\
&\equiv \bar{V}+\Delta V
\end{aligned}
$$
...
假定周期势场的空间变化十分微弱，$\Delta V $ 是小量，电子行为接近自由电子，$\Delta V $ 可作为微扰处理。
...
有限晶格，由玻恩-冯卡门边界条件 $\psi_{\vec{k}}^0(\vec{r}+N_i\vec{a}_i)=\psi^0_{\vec{k}} (\vec{r}) $ 可得：
...
$a(\vec{k})\approx 1 $，其余 $a(\vec{k}+\vec{K}_h) $ 都是小量
...
根据近自由电子近似，当 $\vec{k} $ 矢量落在布里渊区界面上时，电子能量发生突变，形成宽度为 $E_g=2|V(\vec{K}_h) | $ 的能隙。
...
近自由电子近似下的波函数和能谱**不是**倒空间的周期函数，属于不同能带的状态，分布在不同布里渊区内。
...
选取孤立原子的定域波函数 $\varphi_n(\vec{r}-\vec{R}_l) $ 来近似万尼尔函数，$\varphi_n(\vec{r}-\vec{R}_l) $ 满足：
...
其中,$U(\vec{r}-\vec{R}_l) $ 表示 $\vec{R}_l $ 处的元胞在 $\vec{r} $ 处产生的势。
...
\psi_{\vec{k}}^n(\vec{r})
&=\frac{1}{\sqrt{N}}\sum_{\vec{R}_l} a_n(\vec{R}_l,\vec{r})\mathrm{e}^{\mathrm{i}\vec{k}\cdot\vec{R}_l} \\
&\approx \frac{1}{\sqrt{N}}\sum_{\vec{R}_l} \varphi_n(\vec{r}-\vec{R}_l) \mathrm{e}^{\mathrm{i}\vec{k}\cdot\vec{R}_l} \\
&=
\end{aligned}
$$
...
$$
N_n(E)
=\frac{2V}{(2\pi)^3}\int\limits_{\Omega^*} \mathrm{d}^3\vec{k}\delta[E-E_n(\vec{k})]
$$
