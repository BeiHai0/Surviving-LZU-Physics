...
拉格朗日密度，记为 $\mathscr{L} $，表示单位固有体元 $\mathrm{d}\Omega=\mathrm{d}x_1\wedge \mathrm{d}x_2\wedge\mathrm{d}x_3\wedge \mathrm{d}x_4 $ 内的拉氏函数。（$\mathrm{d}x_4=\mathrm{i}ct $）
...
P(t')
&=\frac{q^2 }{16\pi^2\varepsilon_0 c^3 }\oint\frac{\left|\hat{r}\times\left[\left(\hat{r}-\vec{v}/c \right)\times\dot{\vec{v}} \right] \right|^2 }{\left(1-\vec{v}\cdot\hat{r}/c \right)^5 }\mathrm{d}\Omega \\
&\approx \frac{q^2 }{16\pi^2\varepsilon_0 c^3 } \oint \frac{\dot{v}^2 \sin^2\Theta }{1^5 } \mathrm{d}\Omega \\
&=\frac{q^2 \dot{v}^2 }{16\pi^2\varepsilon_0 c^3 } \int_{\varphi=0}^{\varphi=2\pi} \int_{\theta=0}^{\theta=\pi} \sin^2\Theta \cdot \sin\Theta \mathrm{d}\Theta\mathrm{d}\varphi \\
&=\frac{q^2 \dot{v}^2 }{16\pi^2\varepsilon_0 c^3 }\cdot \frac{8\pi }{3 }  \\
&=\frac{q^2\dot{v}^2 }{6\pi\varepsilon_0 c^3 } \\
\end{aligned}
$$

- 低速粒子的辐射功率 $P(t') $ 与速度和能量无关，只取决于加速度 $\dot{v} $。
...
辐射微分能量密度 $\mathrm{d}W/\mathrm{d}\Omega $：
...
当 $\omega $ 离 $\omega_0 $ 较远，$\omega $ 增加，介电常数实部也 增加，正常色散；
...
$$
\vec{M}\cdot\frac{\mathrm{d}\vec{M} }{\mathrm{d}t } 
=0
$$
...
$$
\frac{\mathrm{d} \left|\vec{M} \right|^2 }{\mathrm{d}t } = 0
$$
...
\vec{m}
=\pmb{\chi}\cdot\vec{h}
=\left(\pmb{\mu}-1 \right)\cdot\vec{h}
$$
...
若 $\vec{k} $ 沿 $\vec{H}_e $ 轴，即 $\theta $，则：

$$
\gamma_\pm
=\mathrm{j}\omega \left[\mu_0 \left(\mu\mp\kappa \right)\varepsilon \right]^{1/2}
$$
...
其中，$\displaystyle{\omega_p \equiv \sqrt{\frac{n_0 e^2 }{m \varepsilon_0 } } }$ 称为等离子体频率。
...
$$
\phi_1-\phi_2
=\int\limits_{C_1} \frac{m\vec{v} }{\hbar } \cdot\mathrm{d}\vec{r} - \int\limits_{C_c} \frac{m\vec{v} }{\hbar } \cdot\mathrm{d}\vec{r}
$$
...
求理想迈斯纳态下，超导体表面电流密度 $\vec{\alpha}_s $ 与界面磁感应强度的关系。

边值关系：

$$
\vec{n}_{1\to 2}\times \left(\vec{H}_2-\vec{H}_1 \right) = \vec{\alpha}_s,\quad
\vec{n}_{1\to 2}\cdot \left(\vec{B}_2-\vec{B}_1 \right) = 0
$$
...
$$
\vec{B}_1 = \vec{0},\quad
\vec{H}_1 = \vec{B}_1/\mu_0 = \vec{0}
$$

因此得到：

$$
\vec{n}\times\vec{H}_2 = \vec{\alpha}_s,\quad
B_{2n} = B_{1n} = 0
$$