...
辐射能流密度 $u_\nu $（或 $u_\lambda $）：单位面积的黑体向单位立体角内辐射的单位频率（或单位波长）的电磁波功率。

$$
u_\nu=\frac{c\rho_\nu}{4\pi}（或u_\lambda=\frac{c\rho_\lambda}{4\pi }）
$$

可以看到，辐射能流密度与腔体能流密度只相差一个常系数。
...
- 能量量子 $\varepsilon=h\nu$ , 其中 $h=6.62559\times 10^{-34}\rm{J\cdot s}$ 称作普朗克常数。

> 玻尔兹曼分布：对于服从玻尔兹曼分布且处于热平衡的系统，能级 $E_i $ 的粒子占据数 $N_i\propto \exp\left(-\beta E_i \right) $，其中 $\beta\equiv 1/(k_B T) $，$k_B $ 是玻尔兹曼常数。

普朗克理论的建立：

假设黑体辐射是大量电磁驻波场的集合，且能量量子化，能级分布为 $\left\{\varepsilon_n=n \varepsilon \right\} .$
...
- 电子仅沿特殊轨道进行圆周运动，运动时不产生电磁辐射，此时处于具有确定能量的稳定状态，称为**定态**；

- 电子的特殊运动轨道由玻尔-索莫菲量子化条件 $\displaystyle{\oint p \mathrm{d}q = n h}$  确定，其中 $q $ 是广义坐标，$p $ 是相应的广义动量，$n $ 取正整数；

- 当电子在两个定态 $E_m $ 和 $E_n $ 跃迁时，才伴随频率 $\nu=\left|E_n-E_m \right|/h $ 的电磁波的吸收或发射；

### 玻尔-索莫菲量子化条件给出的氢原子定态轨道
...
\Longrightarrow 
p
=\sqrt{2mE} \sqrt{1-\frac{m \omega^2 }{2E } x^2}
$$

经典谐振子 $x\in \left[x_{\min} , x_{\max} \right],x_{\min}=0,x_{\max}=\sqrt{2E/m\omega^2} .$

> 注意这里回路积分意味着要对质点一个周期内的运动轨迹积分。

$$
\begin{aligned}
n h
&=\oint p \mathrm{d}x \\
&=4 \int_{x=x_{\min}}^{x=x_{\max}} \sqrt{2mE} \sqrt{1-\frac{m \omega^2 }{2E } x^2} \mathrm{d}x \\
&=4 \sqrt{2mE}\int_{x=0}^{x=\sqrt{2E/m\omega^2}} \sqrt{1-\frac{m \omega^2 }{2E } x^2} \mathrm{d}x \\
\left(令 \cos\theta=\sqrt{\frac{m\omega^2 }{2E }} x \right)&=4 \sqrt{2mE}\int_{\theta=\pi/2}^{\theta=0} \sin\theta \cdot \left(-\sqrt{\frac{2 E }{m \omega^2 } } \sin\theta \right) \mathrm{d}\theta \\
&=8 \frac{E }{\omega }  \int_{\theta=0}^{\theta=\pi/2} \sin^2\theta \mathrm{d}\theta \\
&=8 \frac{E }{\omega }  \cdot \frac{1 }{2 } \frac{\pi }{2 } \\
&=2\pi \frac{E }{\omega } 
\end{aligned}
$$

$$
2\pi \frac{E }{\omega } = n h \Longrightarrow E_n = n \frac{h }{2\pi } \omega = n \hbar \omega
$$

可以看到，玻尔-索莫菲量子化条件没有给出实际上量子谐振子所具有的零点能，这是其局限性。然而，虽不中，亦不远矣。
...
密封的盒子里有一只猫和一个放射性原子，原子有 $50\% $ 概率衰变。若原子衰变，则通过装置释放毒药毒死猫。若原子不衰变，则猫或者。问猫是死是活？

波函数的概率解释给出，猫处于 $50\% $ 概率死和 $50\% $ 概率活的叠加态。

打开盒子相当于进行一次测量，体系状态会塌缩到本征态，从而看到猫要么死，要么活。
...
> 或者用 Dirac 符号写为 $\Braket{\phi | \hat{O} \psi } = \Braket{\hat{O}^\dag \phi | \psi} $
