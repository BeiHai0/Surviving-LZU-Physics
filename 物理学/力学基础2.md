...
选取 $F_1(q,Q,t) $ 满足 $(1) $ 式，即：

$$
\boxed{
\mathrm{d}F_1
=p_i\mathrm{d}q_i - P_i\mathrm{d}Q_i - \left(H-H' \right)\mathrm{d}t
}
$$
...
$$
p_i = \frac{\partial F_1 }{\partial q_i } ,\quad
P_i = -\frac{\partial F_1 }{\partial Q_i } ,\quad
\frac{\partial F }{\partial t } = -(H-H')
$$

可见，$F_1 $ 关于 $q_i $ 的共轭动量是 $p_i $，关于 $Q_i $ 的共轭动量是 $-P_i .$

#### $F_2(q,P,t) $

对 $F_1(q,Q,t) $ 关于 $Q $ 作勒让德变换（差个负号不影响），就得到 $F_2(q,P,t) $：

$$
F_2(q,P,t)
=-\frac{\partial F_1 }{\partial Q_i } Q_i + F_1
=P_i Q_i +F_1
$$

$$
\mathrm{d}F_2
=P_i\mathrm{d}Q_i + Q_i\mathrm{d}P_i + \mathrm{d}F_1
=p_i\mathrm{d}q_i + Q_i\mathrm{d}P_i - \left(H-H' \right)\mathrm{d}t
$$

偏微分关系为：

$$
\frac{\partial F_2 }{\partial q_i } = p_i,\quad
\frac{\partial F_2 }{\partial P_i } = Q_i,\quad
\frac{\partial F_2 }{\partial t } = -(H-H')
$$

#### $F_3(p,Q,t) $

对 $F_1(q,Q,t) $ 关于 $q $ 作勒让德变换，就得到 $F_3(p,Q,t) $：
...
\mathrm{d}F_3
=p_i\mathrm{d}q_i + q_i\mathrm{d}p_i - \mathrm{d}F_1
=q_i\mathrm{d}p_i + P_i\mathrm{d}Q_i + \left(H-H' \right)\mathrm{d}t
$$

偏微分关系为：

$$
\frac{\partial F_3 }{\partial p_i } = q_i,\quad
\frac{\partial F_3 }{\partial Q_i } = P_i,\quad
\frac{\partial F_3 }{\partial t } = (H-H')
$$

#### $F_4(p,P,t) $

对 $F_1(q,Q,t) $ 关于 $q,Q $ 作勒让德变换（可能会与其他书差个负号），就得到 $F_4(p,P,t) $：

$$
F_4(p,P,t)
=\frac{\partial F_1 }{\partial q_i } q_i +\frac{\partial F_1 }{\partial Q_i } Q_i - F_1
=p_i q_i - P_i Q_i - F_1
$$

$$
\mathrm{d}F_4
=p_i\mathrm{d}q_i + q_i\mathrm{d}p_i - P_i\mathrm{d}Q_i - Q_i\mathrm{d}P_i - \mathrm{d}F_1
=q_i\mathrm{d}p_i - Q_i\mathrm{d}P_i + (H-H')\mathrm{d}t
$$

偏微分关系为：

$$
\frac{\partial F_4 }{\partial p_i } = p_i,\quad
\frac{\partial F_4 }{\partial P_i } = -Q_i,\quad
\frac{\partial F_4 }{\partial t } = -(H-H')
$$

---

判断一个变换 $(p,q)\to(P,Q)$ 是否为正则变换的三种方法：
