...
拉格朗日量 $\mathcal{L} $ 是广义坐标和广义速度的函数：

$$
\mathcal{L}
=\mathcal{L}\left(q_1,\cdots,q_n;\dot{q}_1,\cdots,\dot{q}_n \right)
$$
...
拉格朗日量关于广义速度 $\dot{q}_1,\cdots,\dot{q}_n $ 的勒让德变换称为哈密顿量 $\mathcal{H}(q_1,\cdots,q_n;p_1,\cdots,p_n) $，定义为：

$$
\mathcal{H}\left(q_1,\cdots,q_n;p_1,\cdots,p_n \right)
\equiv \sum_{i=1}^{n} p_i \dot{q}_i(p_i) - \mathcal{L}\left(q_1,\cdots,q_n;\dot{q}_1(p_1),\cdots,\dot{q}_n(p_n) \right),\quad
...
$$
\mathrm{d}\mathcal{H}\left(q_1,\cdots,q_n;p_1,\cdots,p_n \right)
=\frac{\partial\mathcal{H} }{\partial q_i } \mathrm{d}q_i + \frac{\partial\mathcal{H} }{\partial p_i } \mathrm{d}p_i
$$
...
\begin{aligned}
\mathrm{d}\mathcal{H}\left(q_1,\cdots,q_n;p_1,\cdots,p_n \right)
&=\sum_{i=1}^{n} \left[\dot{q}_i(p_i)\mathrm{d}p_i + p_i\mathrm{d}\dot{q}_i(p_i) \right] - \sum_{i=1}^{n} \left(\frac{\partial\mathcal{L} }{\partial q_i }\bigg|_{\dot{q}_i=\dot{q}_i(p_i)} \mathrm{d}q_i + \frac{\partial\mathcal{L} }{\partial \dot{q}_i }\bigg|_{\dot{q}_i=\dot{q}_i(p_i)} \mathrm{d}\dot{q}_i(p_i) \right) \\
&=\sum_{i=1}^{n} \left[\dot{q}_i(p_i)\mathrm{d}p_i + p_i\mathrm{d}\dot{q}_i(p_i) \right] - \sum_{i=1}^{n} \left(\frac{\partial\mathcal{L} }{\partial q_i }\bigg|_{\dot{q}_i=\dot{q}_i(p_i)} \mathrm{d}q_i + p_i \mathrm{d}\dot{q}_i(p_i) \right) \\
&=\sum_{i=1}^{n}\left[\dot{q}_i(p_i)\mathrm{d}p_i - \frac{\partial\mathcal{L} }{\partial q_i }\bigg|_{\dot{q}_i=\dot{q}_i(p_i)} \mathrm{d}q_i \right]
\end{aligned}
$$
