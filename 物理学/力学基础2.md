...
$$
\mathcal{H}\left(q,p,t \right)
=\dot{q}_i p_i - \mathcal{L}\left(q,\dot{q}\left(p \right),t \right)
$$

考虑有 $n $ 个自由度的体系，选取 $n $ 个广义坐标 $q_1,\cdots,q_n $，对应有 $n $ 个广义速度 $\dot{q}_1,\cdots,\dot{q}_n $ 和 $n $ 个广义动量 $\dot{p}_1,\cdots,p_n $。

可以写出系统的拉式量

$$
\mathcal{L}
=\mathcal{L}\left(q_1\cdots,q_n;\dot{q}_1,\cdots,\dot{q}_k,\dot{q}_{k+1} \cdots \dot{q}_n; t \right)
$$

我们知道，对拉式量作勒让德变换就得到哈密顿量，这其实是指对拉式量中所有的广义速度作勒让德变换就得到哈密顿量。

现在只对 $\dot{q}_{1},\cdots,q_k $ 作勒让德变换，就得到罗斯函数：

$$
\mathcal{R}\left(q_1,\cdots,q_n;p_1,\cdots,p_k;\dot{q}_{k+1},\cdots,\dot{q}_n ; t \right)
\equiv \sum_{i=1}^{n} p_i \dot{q}_i\left(p_i \right) - \mathcal{L}\left(q_1\cdots,q_n;\dot{q}_1(p_1),\cdots,\dot{q}_k(p_k),\dot{q}_{k+1} \cdots \dot{q}_n; t \right)
$$
