...
设想大量结构完全相同的系统，处在相同的宏观条件下，把这些大量系统的集合称为**统计系综**。这些大量系统按分布函数 $\rho(q,p,t) $ 在相空间中分别。也就是说，相空间中 $\rho $ 大的地方，附近的代表点也就多一些。
...
系统和热源构成一个复合系统，这个复合系统是孤立系统，具有确定的粒子数 $N_0 $ 和确定的能量 $E_0 $

用 $E_s $ 和 $E_r $ 表示系统和热源的能量，$N $ 和 $N_r $ 表示系统和热源的粒子数。假设系统和热源的相互作用很弱，则有：
...
$$
\frac{E_s }{E_0 } \ll 1,\quad
\frac{N }{N_0 } \ll 1
$$

当系统处于粒子为 $N $，能量为 $E_s $ 的微观状态 $s $ 时，热源可处在粒子数为 $N_0-N $，能量为 $E_0-E_s $ 的任何一个微观状态。

用函数 $\Omega_r\left(N_r,E_r \right) $ 表示粒子数为 $N_r $，能量为 $E_r $ 的热源的微观状态数。

用 $\Omega_0(N_0,E_0) $ 表示粒子数为 $N_0 $，能量为 $E_0 $ 的复合系统的微观状态数。

系统具有粒子数 $N $，处在微观状态 $s $ 的概率 $\rho_{N,s} $ 为：

$$
\rho_{N,s}
=\frac{\Omega_r(N_0-N,E_0-E_s) }{\Omega_0(N_0,E_0) } 
\propto \Omega_r(N_0-N,E_0-E_s)
=\mathrm{e}^{\ln \Omega_r(N_0-N,E_0-E_s)}
$$

注意到 $\ln \Omega_r(N_0-N,E_0-E_s)=\ln \Omega_r\left[N_0\left(1-\frac{N }{N_0 }  \right) , E_0\left(1-\frac{E_s }{E_0 }  \right) \right] $，把这个复合函数看作 $N/N_0,E_s/E_0 $ 的二元函数，由于自变量的取值满足 $N/N_0\ll 1,E_s/E_0\ll 1 $，因此将函数在 $\left(N/N_0,E_s/E_0 \right)=(0,0) $ 处进行泰勒展开保留到一阶也有很好的精度：

$$
\begin{aligned}
\ln \Omega_r\left[N_0\left(1-\frac{N }{N_0 }  \right) , E_0\left(1-\frac{E_s }{E_0 }  \right) \right]
&=\ln \Omega_r\left(N_0,E_0 \right) + \frac{\partial \ln \Omega_r\left[N_0\left(1-\frac{N }{N_0 }  \right) , E_0\left(1-\frac{E_s }{E_0 }  \right) \right] }{\partial \left(N/N_0 \right) }\Bigg|_{N/N_0=E_s/E_0=0} \left(\frac{N }{N_0 } \right) + \frac{\partial \ln \Omega_r\left[N_0\left(1-\frac{N }{N_0 }  \right) , E_0\left(1-\frac{E_s }{E_0 }  \right) \right] }{\partial \left(E_s/E_0 \right) }\Bigg|_{N/N_0=E_s/E_0=0} \left(\frac{E_s }{E_0 } \right) \\
&=\ln \Omega_r\left(N_0,E_0 \right) + \frac{\partial \ln \Omega_r\left(N_0-N,E_0-E_s \right) }{\partial N }\bigg|_{N=E_s=0} N + \frac{\partial \ln \Omega_r\left(N_0-N,E_0-E_s \right) }{\partial E_s }\bigg|_{N=E_s=0} E_s \\
&=\ln \Omega_r\left(N_0,E_0 \right) + \frac{\partial \ln \Omega_r\left(N_0-N,E_0 \right) }{\partial N }\bigg|_{N=0} N + \frac{\partial \ln \Omega_r\left(N_0,E_0-E_s \right) }{\partial E_s }\bigg|_{E_s=0} E_s \\
&=\ln \Omega_r\left(N_0,E_0 \right) - \frac{\partial \ln \Omega_r\left(N_0-N,E_0 \right) }{\partial (N_0-N) }\bigg|_{N_0-N=N_0} N - \frac{\partial \ln \Omega_r\left(N_0,E_0-E_s \right) }{\partial (E_0-E_s) }\bigg|_{E_0-E_s=E_0} E_s \\
&=\ln \Omega_r\left(N_0,E_0 \right) - \frac{\partial \ln \Omega_r\left(N_r,E_0 \right) }{\partial N_r }\bigg|_{N_r=N_0} N - \frac{\partial \ln \Omega_r\left(N_0,E_r \right) }{\partial E_r }\bigg|_{E_r=E_0} E_s \\
&=\ln \Omega_r\left(N_0,E_0 \right) - \frac{\partial \ln \Omega_r\left(N_0,E_0 \right) }{\partial N_0 }\cdot N - \frac{\partial \ln \Omega_r\left(N_0,E_0 \right) }{\partial E_0 }\cdot E_s \\
&=\ln\Omega_r(N_0,E_0) - \alpha N - \beta E
\end{aligned}
$$
...
$$
\alpha
\equiv \frac{\partial \ln\Omega_r(N_0,E_0) }{\partial N_0 } 
$$

$$
\beta
\equiv \frac{\partial \ln\Omega_r(N_0,E_0) }{\partial E_0 } 
$$
...
$$
\mathrm{d}S
=\frac{1 }{T } \mathrm{d}E + \frac{p }{T } \mathrm{d}V - \frac{\mu }{T } \mathrm{d}N
$$
...
伊辛模型考虑 $N $ 个磁性原子定域在晶格的格点上，假设每个原子的总角动量量子数为 $1/2 $，原子磁矩大小为 $\mu=e\hbar/2m .$ 对于单轴各向异性铁磁体，其原子的自旋平行或反平行于晶轴。

用 $\sigma_i $ 表示格点 $i $ 上原子的自旋方向，$\sigma_i=+1 $ 代表自旋平行于晶轴，$\sigma_i=-1 $ 代表自旋反平行于晶轴。

伊辛模型的能量包括两个部分：自旋-磁场相互作用能以及自旋-自旋相互作用能。

自旋-磁场相互作用能为：

$$
-B\cdot\left(\sum_i \mu \sigma_i \right)
=-\mu B \sum_i \sigma_i
$$
...
$$
- \frac{1 }{2 } \sum_{ij} J_{ij} \sigma_i \sigma_j
$$

其中 $J_{ij} $ 是格点 $i,j $ 交换作用常量。约定求和对 $i,j $ 所有可能取值求和，$J_{ij}=J_{ji} $，因此有个 $1/2 $ 去重复。约定自己与自己没有交换作用，则 $J_{ii}=0. $ 

系统的总能量取决于 $N $ 个自旋的取向 $\left\{\sigma_1,\cdots,\sigma_N \right\}\equiv \left\{\sigma_i \right\} $：

$$
E\left\{\sigma_i \right\}
=-\mu B \sum_i \sigma_i - \frac{1 }{2 } \sum_{ij} J_{ij} \sigma_i \sigma_j
$$
...
E\left\{\sigma_i \right\}
&=-\mu B \sum_i \sigma_i - \frac{1 }{2 } \sum_{ij} J_{ij} \sigma_i \sigma_j \\
&=-\sum_i \mu \sigma_i \left(B+\frac{1 }{2\mu } \sum_j J_{ij} \sigma_j \right)
\end{aligned}
$$
...
$$
B_i
\equiv B+\frac{1 }{2\mu } \sum_j J_{ij} \sigma_j
$$
...
$$
E\left\{\sigma_i \right\}
=-\sum_i \mu \sigma_i B_i
$$

### 伊辛模型的平均场近似

平均场近似就是把 $B_i $ 用其平均值 $\bar{B}_i $ 来代替从而近似体系的能量。

$$
\begin{aligned}
\bar{B}_i
=B + \frac{1 }{2\mu } \sum_j J_{ij} \bar{\sigma}_j
\end{aligned}
$$

假设体系具有平移对称性，则 $\bar{\sigma}_i=\bar{\sigma}_j\equiv \bar{\sigma} $，再假设交换相互作用只发生在最近邻原子之间，且任意两个最近邻原子的交换作用常数都相等，即 $J_{ij}\equiv J,i\ne j $，设近邻自旋数为 $z $，则：

$$
\begin{aligned}
\bar{B}_i
&=B + \frac{1 }{2\mu } \sum_j J_{ij} \bar{\sigma}_j \\
&=B + \frac{1 }{2\mu } J z \bar{\sigma} \\
&\equiv \bar{B}
\end{aligned}
$$
...
E\left\{\sigma_i \right\}
&=-\sum_i \mu \sigma_i B_i \\
&\approx -\sum_i \mu \sigma_i \bar{B} \\
&=-\mu \bar{B} \sum_i \sigma_i
\end{aligned}
$$
...
\begin{aligned}
Z
&=\sum_{\left\{\sigma_i \right\}} \exp\left(-\beta E\left\{\sigma_i \right\} \right) \\
&=\sum_{\sigma_1=\pm 1} \cdots \sum_{\sigma_N=\pm 1} \exp\left(\beta \mu \bar{B} \sum_i \sigma_i \right) \\
&=\sum_{\sigma_1=\pm 1}\exp\left(\beta \mu \bar{B} \sigma_1 \right) \cdots \sum_{\sigma_N=\pm 1} \exp\left(\beta \mu \bar{B} \sigma_N \right) \\
&=\prod_{i=1}^{N} \sum_{\sigma_i=\pm 1}\exp\left(\beta\mu\bar{B}\sigma_i \right)
\end{aligned}
$$
...
$$
\mathscr{M}
=\frac{1 }{\beta } \frac{\partial \ln Z }{\partial B } 
=N \mu \tanh \left(\frac{\mu \bar{B} }{k T }  \right)
$$

温度 $T $ 时 $\bar{\sigma} $ 由下式确定：

$$
\bar{\sigma}
=\frac{\mathscr{M} }{\mu N } 
=\tanh \left(\frac{\mu \bar{B} }{k T }  \right)
=\tanh \left[\frac{\mu }{k T }\left(B + \frac{1 }{2\mu } J z \bar{\sigma} \right)  \right]
$$

特别地，当外磁场 $B=0 $ 时，$\bar{\sigma} $ 由下面超越方程给出：

$$
\bar{\sigma}
=\tanh\left[\frac{J z }{2 kT } \bar{\sigma} \right]
$$

当 $Jz/(2kT)>1 $，即 $T<Jz/(2k) $ 时，方程解为 $\bar{\sigma}=\pm \bar{\sigma}_0,\bar{\sigma}_0>0 $，此时体系存在自发磁化。

当 $Jz/(2kT)<1 $，即 $T>Jz/(2k) $ 时，$\bar{\sigma}=0 $，此时不存在自发磁化。
...
局域平衡假设：对于一个处于非平衡态的系统，可以在系统中取一个宏观小、微观大的体积元，在任一时刻该体积元内的子系统可看成是平衡的。

用 $f(\vec{r},\vec{v},t)\mathrm{d}^3\vec{r}\mathrm{d}^3\vec{v} $ 表示 $t $ 时刻 $\vec{r} $ 处体积元 $\mathrm{d}^3\vec{r} $ 内，速度在 $\vec{v}\sim \vec{v}+\mathrm{d}\vec{v} $ 范围内的气体分子数目。

经过一系列假设和推导，可以得到玻尔兹曼积分微分方程：

$$
\frac{\partial f }{\partial t } + \nabla_{\vec{r}}\cdot\left(f\dot{\vec{r}} \right) + \nabla_{\vec{v}}\cdot\left(f\dot{\vec{v}} \right)
=\int \left(f_1' f' - f_1 f \right)\mathrm{d}^3\vec{v}_1 \Lambda \mathrm{d}\Omega
$$

其中 $f_1\equiv f(\vec{r},\vec{v}_1,t),f_1'\equiv \left(\vec{r},\vec{v}_1',t \right),\Lambda\equiv R_{12}^2 v_r \cos\theta $
...
$$
H(t)
\equiv \int f\left(\vec{r},\vec{v},t \right)\ln f(\vec{r},\vec{v},t)\mathrm{d}^3\vec{r}\mathrm{d}^3\vec{v}
$$

可以证明

$$
\frac{\mathrm{d}H }{\mathrm{d}t } \leqslant 0
$$

上式就是玻尔兹曼 H 定理。

等号成立当且仅当 $f f_1 = f' f_1' $，这个条件称为**细致平衡条件**。

细致平衡条件也可表达为：

$$
W_{i\to j} P_i^{\mathrm{eq}}
=W_{j\to i} P_j^{\mathrm{eq}}
$$

其中 $W_{i\to j} $ 是态 $i\to j $ 的跃迁概率。$P_i^{\mathrm{eq}} $ 是平衡态下系统处于 $i $ 态的概率。
