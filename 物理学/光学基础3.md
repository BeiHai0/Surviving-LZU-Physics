...
算术基本定理：每个大于 1 的整数都可以唯一地表示为若干个质数的乘积。

把一个合数 $N $ 分解为两个数之积的方法：

- 选择 $a<N $，计算 $\mathrm{gcd}(a,N) $，其中 $\mathrm{gcd}(\cdot,\cdot) $ 表示括号内两个数的最大公因数；

- 若 $\mathrm{gcd}(a,N)\ne 1 $，则 $a $ 是 $N $ 的一个因数，则问题解决；

- 若 $\mathrm{gcd}(a,N) = 1 $，则寻找 $f(x)=a^x \mathrm{mod} N $ 的周期 $r $；

- 若 $r $ 为奇数，则返回第一步；

- 若 $r $ 为偶数，则 $\mathrm{gcd}\left(a^{r/2}\pm 1 , N \right) $ 为 $N $ 的两个因数。

举个例子，假设想把 $N=15 $ 分解为两个因数的乘积。选择 $a=7 $，显然 $N,a $ 的最大公因数为 $1 $，因此构造函数：
...
则 $r=4 $，这是一个偶数，则 $\mathrm{gcd}\left(a^{r/2}\pm 1 , N \right)=\mathrm{gcd}\left(7^{4/2}\pm 1 , 15 \right)=5,3 $ 为 $N=15 $ 的两个因数。

对于大数来说，想将其一直分解，直到所有因数都是指数的话需要非常长的时间。因此认为大数不可因子分解。
...
$$
\Ket{\Phi_0 }
=Q^{-1/2} \sum_{x=0}^{Q-1} \Ket{x }\rangle \Ket{0 }\rangle
$$

其中，

$$
N^2 \leqslant Q = 2^L \leqslant 2N^2,\quad
L\in \mathbb{N}
$$
...
$$
\Ket{\Phi_1 }
=Q^{-1/2} \sum_{x=0}^{Q-1} \Ket{x }\rangle \Ket{f(x) }\rangle
$$
...
有一未知黑盒 $f:\left\{0,1 \right\}^n \to \left\{0,1 \right\} $，$f(x) $ 只有两种可能：

- 常函数：$\forall x,f(x)=1~\mathrm{or}~0 $；

- 均匀函数：一半的 $x $ 使得 $f(x)=0 $，另一半的 $x $ 使得 $f(x)=1 $

想要知道 $f(x) $ 是哪种函数。

经典计算机至少调用 $f $ 至少 $2^n/2+1 $ 次。
...
有一未知黑盒 $f:\left\{0,1 \right\}^n \to \left\{0,1 \right\} $，想要找出使得 $f(x)=1 $ 的所有 $x $

经典计算机需要调用 $f $ 至少 $N=2^n $ 到 $N/2 $ 次。

量子计算机调用 $f $ 的次数在 $\sqrt{N} $ 量级。
