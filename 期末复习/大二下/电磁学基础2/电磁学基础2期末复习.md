...
\left\{
\begin{aligned}
&x'_1 = \gamma(x_1 + \mathrm{i} vx_4/c) \\
&x'_2 = x_2 \\
&x'_3 = x_3 \\
&x'_4 = \gamma\left(x_4 - \mathrm{i} vx_1/c\right)
\end{aligned}
\right.
$$
...
a
=\begin{bmatrix}
\gamma &0 &0 &\mathrm{i}\frac{v\gamma }{c } \\
0 &1 &0 &0 \\
0 &0 &1 &0 \\
-\mathrm{i}\frac{v\gamma }{c } &0 &0 &\gamma
\end{bmatrix}
$$
...
$$
F_{14}'
=a_{1\alpha} a_{4\beta} F_{\alpha\beta}
=a_{11} a_{44} F_{14} + a_{14} a_{41} F_{41}
=\left(a_{11}a_{44}-a_{14}a_{41} \right)F_{14}
=\left(\gamma^2 - v^2\gamma^2/c^2 \right)F_{14}
=F_{14}
$$

$$
F_{23}'
=a_{2\alpha} a_{3\beta} F_{\alpha\beta}
=a_{22} a_{33} F_{23} 
=F_{23}
$$
...
$$
E_2'=\gamma(E_2-vB_3),\quad 
B_2'=\gamma\left(B_2+\frac{v }{c^2 } E_3 \right)
$$

$$
E_3'=\gamma(E_3+vB_2),\quad
B_3'=\gamma\left(B_3-\frac{v }{c^2 } E_2 \right)
$$
...
$$
E'_\perp = \gamma\left(\vec{E}+\vec{v}\times\vec{B} \right)_\perp,\quad
B'_\perp = \gamma\left(\vec{B}-\frac{\vec{v} }{c^2 } \times\vec{E} \right)_\perp,\quad
$$

若电场和磁场在 $\Sigma $ 系都不为零，则可找到一个 $\Sigma' $ 惯性系使得电场或磁场在 $\Sigma' $ 为零。但不可能通过洛伦兹变换使得二者同时为零。

可以证明，$\vec{E}\cdot\vec{B} $ 是洛伦兹不变量，即 $\vec{E}'\cdot\vec{B'}=\vec{E}\cdot\vec{B} $，因此不可能通过洛伦兹变换使电场和磁场的夹角从锐角变为钝角。