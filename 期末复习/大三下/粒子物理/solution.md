...
> 请说明实验上如何发现下列粒⼦（任选5个）：（1）电⼦；（2）µ轻⼦；（3）τ轻⼦；（4）π介⼦；（5）粲夸克；（6）底夸克；（7）顶夸克；（8）中微⼦；（9）W玻⾊⼦；（10）Z玻⾊⼦；（11）希格斯粒⼦；（12）胶⼦。

# 二

> 请说明下⾯那些过程是可以发⽣并通过何种相互作⽤发⽣。如果某过程被禁戒，请说明原因。
...
> 实验上观测到强衰变过程 $\rho^0 \to \pi^+ \pi^- $，但没有观测到 $\pi^0 \to \pi^0 \pi^0 $ 衰变。请据此推导出 $\rho $ 的量子数：（1）$G $ 宇称；（2）自旋；（3）内禀宇称，并提供 $\rho^0 \to \pi^0 \pi^0 $ 衰变过程的三种不同原因。
...
> 在未来质⼼系能量为 $1000~\mathrm{GeV} $ 的正负电⼦直线加速器上，我们期望可观测到⼀对正反顶夸克事例
...
考虑下列各种新物理奇异粒⼦，假设这些奇异粒⼦仅参与QED相互作⽤。当正负电⼦对撞机具有⾜够⾼的能量可以产⽣⼀对新物理粒⼦时，请计算相应的 $R $ 值：

(1) $R_L $：$\mathrm{SU}(2) $ 弱同位旋二重态的新轻⼦ $L $，设其超荷为 $Y=+3 $
...
仅考虑QED理论。在非极化的正负电⼦对撞机上，忽略电⼦质量时，$e^+ e^- \to \tau^+ \tau^- $ 的散射振幅模方为：
