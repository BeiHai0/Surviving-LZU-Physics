...
\begin{frame}
    ...
    其中，$a$ 是依赖于时间的尺度因子，表示某时刻相对参考时刻宇宙的线性尺度变化。相应 $h_\mu^a=\mathrm{diag}(1,a,a,a)$。$f(T)$ 引力中，修改 Friedmann 方程描述了均匀且各向同性宇宙的演化:
    ...
    其中，$H=\dot{a}/a$ 是哈勃参数，描述了宇宙的膨胀速度；$\rho_t,p_t$ 分别是宇宙的总能量密度和压力。
\end{frame}
...
\begin{frame}
    ...
    负的 $q$ 意味着宇宙膨胀在加速，正的 $q$ 意味着宇宙膨胀在减速。
...
\begin{frame}
    ...
    该模型满足在高红移下 $f_T\to 0$ 的条件，符合宇宙加速膨胀的要求。

    对于较小的红移值 $z$，$q_t<0$，宇宙处于加速膨胀状态，这与现实一致，也与原始核合成(primordial nucleosynthesis)和宇宙微波背景约束一致。
...
\begin{frame}{Hubble Horizon}
    ...
    对于哈勃视界，当 $z>8.2$ 或 $z<0$ 时 GSLT 成立。
...
\begin{frame}{Event Horizon}
    ...
    对于事件视界，有同样的结果，当 $z>8.2$ 或 $z<0$ 时 GSLT 成立。

\end{frame}

\begin{frame}{另一种形式的尺度因子}

    构造幂律型尺度因子 $a(t)=a_0(t_s-t)^h$，与之对应的 $f(T)$ 模型
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.6\textwidth]{img/fig4.png}
...
\begin{frame}
    ...
    对于幂律型尺度因子，GSLT 在 Hubble 视界上的有效性取决于参数 $h$ 的取值。对于 $h=5$，GSLT 对所有红移值 $z$ 都成立；而对 $h=2,3$，GSLT 只在特定红移值范围内成立。而对于事件视界，GSLT 对所有的 $h$ 值和红移值 $z$ 都成立。
