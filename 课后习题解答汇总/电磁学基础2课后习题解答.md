...
$$
\nabla\times\vec{E}
=\mathrm{i}\omega\mu_0\vec{H},~~
\nabla\times\vec{H}
=-\mathrm{i}\omega\varepsilon_0\vec{E}
$$

其中，$\vec{E},\vec{H} $ 可以是含时场，也可以不是。

将 $\vec{E}=\vec{E}_0(x,y)\mathrm{e}^{\mathrm{i}(k_z z-\omega t)},~~\vec{H}=\vec{H}_0(x,y)\mathrm{e}^{\mathrm{i}(k_z z-\omega t)} $ 代入上式：
...
$$
\begin{aligned}
-\mathrm{i}\omega\varepsilon_0 E_z
